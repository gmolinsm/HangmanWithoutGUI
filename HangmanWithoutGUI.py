...
class UI:
    def __init__(self, window):
        ...
        self.solution_font = ("Times New Roman", 30)
        alphabet_font = ("Times New Roman", 10)
        buttons_font = ("Times New Roman", 20)
        ...
        # Text
        self.lbl1 = Label(master=window, text="Hangman", font=title_font)
        self.lbl1.grid(row=0, column=1)

        self.lbl2 = Label(master=window, text=self.view_progress, font=self.solution_font)
        self.lbl2.grid(row=3, column=1)

        self.lbl3 = Label(master=window, text=self.wrong_letters + " " + str(self.lives-1), font=self.solution_font)
        ...
        for i in range(0, len(self.alphabet)):
            self.buttons.append(Button(master=window, text=self.alphabet[i], width=2,
                                       font=alphabet_font, command=self.check)) # ????????????????????????
            self.gridpos.append(self.buttons[i].grid(row=1, column=count))
            count += 1

        close = Button(master=window, text="Exit", width=6, font=buttons_font, command=self.close)
    ...
    def check(self):
        ...
        if correct:
            ...
            if self.lives == 0:
                ...
                print(self.view_progress)
                print()
        self.lbl2["text"] = self.view_progress
